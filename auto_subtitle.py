...
print("开始转录...")
segments, info = model.transcribe("{video_path_fixed}", language="{language if language else 'auto'}")
...
def burn_subtitles(video_path, srt_path, output_path=None):
    ...
    """
    # 使用与 processor.py 相同的 FFmpeg
    ffmpeg_path = r"D:\openclaw\workspace\video-downloader\bin\ffmpeg-7.1-essentials_build\bin\ffmpeg.exe"
    ffprobe_path = r"D:\openclaw\workspace\video-downloader\bin\ffmpeg-7.1-essentials_build\bin\ffprobe.exe"
    ...
    # 确保字幕文件也在同一目录
    if os.path.dirname(srt_path) != video_dir:
        import shutil
        new_srt_path = os.path.join(video_dir, srt_name)
        shutil.copy2(srt_path, new_srt_path)
        srt_name = os.path.basename(new_srt_path)
        print(f"  已复制字幕到视频目录")
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"[ERROR] 字幕烧录失败: {result.stderr[-500:]}")  # 只显示最后500字符
            return None
            
    except subprocess.TimeoutExpired:
        print(f"[ERROR] 字幕烧录超时")
        return None
    except Exception as e:
